...
function TickCandleChart({ prices=[], color=C.ac, height=220, barSize=5, showEMA=true, showBB=false }) {
  ...
  const e5  = showEMA ? closes.map((_,i)=>i>=4 ?calcEMA(closes.slice(0,i+1),5):null) : [];
  const e10 = showEMA ? closes.map((_,i)=>i>=9 ?calcEMA(closes.slice(0,i+1),10):null) : [];
...
function RSIChart({ prices=[], height=70 }) {
  ...
  for (let i=14;i<=prices.length;i++) {
    const v = calcRSI(prices.slice(0,i), 14);
    if (v!==null) rv.push(v);
  }
